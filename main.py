...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, num_actions):
        ...
        kernel_size = 5
        stride = 2
        self.first_layer = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=kernel_size, stride=stride)
        h1 = (h-(kernel_size-1)-1)//stride + 1
        w1 = (w-(kernel_size-1)-1)//stride + 1
        self.second_layer = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=kernel_size, stride=stride)
        h2 = (h1-(kernel_size-1)-1)//stride + 1
        w2 = (w1-(kernel_size-1)-1)//stride + 1
        self.third_layer = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=kernel_size, stride=stride)
        h3 = (h2-(kernel_size-1)-1)//stride + 1
        w3 = (w2-(kernel_size-1)-1)//stride + 1

        self.first_layer_norm = nn.BatchNorm2d(16)
        self.second_layer_norm = nn.BatchNorm2d(32)
        self.third_layer_norm = nn.BatchNorm2d(32)

        l_in = w3*h3*32
        l_out = num_actions
        self.final = nn.Linear(l_in, l_out)
        ### WRITE YOUR CODE ABOVE HERE
    ...
    def forward(self, x):
        ...
        q_values = self.final(x.view(x.size(0), -1))
        ### WRITE YOUR CODE ABOVE HERE
        return q_values
...
def select_action(state, policy_net, num_actions, epsilon, steps_done = 0, bootstrap_threshold = 0):
    ...
    if random_number <= epsilon:
        ...
        # query the policy for action
        largest_col = policy_net(state).max(1)
        largest_index = largest_col[1]
        action = largest_index.view(1, 1)
    ...
    if steps_done > bootstrap_threshold:
        ...
        min_eps = 0.05
        decay_rate = 2000
        new_epsilon = min_eps + (epsilon-min_eps) * math.exp(-1.0*steps_done/decay_rate)
    ### WRITE YOUR CODE ABOVE HERE
...
def doMakeBatch(replay_memory, batch_size):
    ...
    # ('state', 'action', 'next_state', 'reward'))
    replay_data = Transition(*zip(*replay_data))
    states = []
    for state in replay_data.state:
        states.append(state)
    states_batch = torch.tensor(torch.cat(states), device=DEVICE)
    actions = []
    for action in replay_data.action:
        actions.append(action)
    actions_batch = torch.tensor(torch.cat(actions), device=DEVICE)
    next_states = []
    for next_state in replay_data.next_state:
        if next_state is not None:
            next_states.append(next_state)
    next_states_batch = torch.tensor(torch.cat(next_states), device=DEVICE)
    rewards = []
    for reward in replay_data.reward:
        rewards.append(reward)
    rewards_batch = torch.tensor(torch.cat(rewards), device=DEVICE)
    #TODO: change this
