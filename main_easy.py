...
BATCH_SIZE = 64#128            # How many replay experiences to run through neural net at once
...
REPLAY_CAPACITY = 2000000#10000     # How big is the replay memory
EPSILON = 0.9               # Use random action if less than epsilon [0..1]
EVAL_INTERVAL = 10          # How many episodes of training before evaluation
RANDOM_SEED = 4623#None          # Seed for random number generator, for reproducability, use None for random seed
...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, num_actions):
        ...
        self.fc = nn.Linear(self.out3_w * self.out3_h * hid_dim3, num_actions)
        ### WRITE YOUR CODE ABOVE HERE
    ...
    def forward(self, x):
        ...
        q_values = self.fc(x.view(x.size(0), -1))
        ### WRITE YOUR CODE ABOVE HERE
        return q_values
...
def select_action(state, policy_net, num_actions, epsilon, steps_done = 0, bootstrap_threshold = 0):
    ...
    if steps_done > bootstrap_threshold:
        new_epsilon = 0.05 + (0.9 - 0.05) * \
            math.exp(-1. * steps_done / 20000)#15000)
    ### WRITE YOUR CODE ABOVE HERE
